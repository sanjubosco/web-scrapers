...
class fileloc(object):   
    ...
    LEGUE_STADNING_FILE_EXT = '_Standing'  
    ORIGINAL_FIXTURE_FILE = 'fixtures.csv'
    ...
    def __init__(self):
        if not (self.initialised):
            ...
            if not (os.path.isdir(os.path.join(self.DATA_LOC, self.DOWNLOADS_LEAGUE_STANDING))):
                ## create the directory
                os.mkdir(os.path.join(self.DATA_LOC, self.DOWNLOADS_LEAGUE_STANDING))
    ...
    def get_league_standing_dir(self):
        return (os.path.join(self.DATA_LOC, self.DOWNLOADS_LEAGUE_STANDING))

    def get_chrome_driver(self):
        return (os.path.join(self.HOME_LOC,self.CHROME_DRIVER))
    
    def get_fixture_file(self):
        return (self.ORIGINAL_FIXTURE_FILE)
...
def get_fs_team_alias(country, team):
    try:
        team = team.lower()
        country = country.upper()
        return team
